...
// Function to perform a recursive in-order traversal of the binary tree
void inorder_r(node *root) {
    if (root == NULL)  // Base case: if the node is NULL, return
        return;

    inorder_r(root->left);    // Recursively traverse the left subtree
    printf("%d ", root->key); // Print the key value of the current node
    inorder_r(root->right);   // Recursively traverse the right subtree
}
