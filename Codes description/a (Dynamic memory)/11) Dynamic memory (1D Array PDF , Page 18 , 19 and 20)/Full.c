...
int main() {
    int *array = NULL; // Pointer to store the array
    int n = 0;         // Variable to store the number of elements

    // Call the read_and_store function to read data from the file
    read_and_store(&array, &n);
...
6. **`main()`**: The `main` function calls `read_and_store` to read the data from the file. It checks if the data was successfully read, prints the elements, and frees the allocated memory. This function serves as the entry point for the program and demonstrates how to use `read_and_store`.
